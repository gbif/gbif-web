...
class TaxonMediaAPI extends RESTDataSource {
  ...
  async getRepresentativeImages({ taxon, size = 1, from, params, dataSources }) {
    ...
    const inatOccurrence = await dataSources.occurrenceAPI.searchOccurrenceDocuments({
      query: {
        limit: 1,
        predicate: {
          type: 'and',
    ...
    try {
      if (inatTaxonId) {
        ...
        const inatTaxa = (await axios.get(
          `https://api.inaturalist.org/v1/taxa/${inatTaxonId}`,
        )).data;

        const properlyLicencedImages = (inatTaxa?.results[0].taxon_photos || [])
          .filter(x => ['cc0', 'cc-by-nc', 'cc-by', 'cc-by-nc-sa', 'cc-by-nc-sa', 'cc-by-nc-nd'].includes(x?.photo?.license_code) || (x?.photo?.license_code || '').startsWith('cc-by'))
    ...
    const occurrenceMedia = await dataSources.occurrenceAPI.searchOccurrenceDocuments({
      query: {
        limit: size,
        predicate: {
          type: 'and',
    ...
    return occurrenceMedia.results.map(x => ({
