...
export default async function colSuggest({q, datasetKey = '53147', language = 'eng', limit = 20, taxonScope = []}) {
  ...
  const nameSuggestions = nameResponse.then(response => {
    ...
    const taxonPromises =               suggestions.map(s => axios.get(`https://api.checklistbank.org/dataset/${datasetKey}/taxon/${s.acceptedUsageId ?? s.usageId}`));
    const taxonClassificationPromises = suggestions.map(s => axios.get(`https://api.checklistbank.org/dataset/${datasetKey}/taxon/${s.acceptedUsageId ?? s.usageId}/classification`));
  ...
  const vernacularNames = vernacularResponse.then(response => {
    ...
    const taxonPromises =               suggestions.map(s => axios.get(`https://api.checklistbank.org/dataset/${datasetKey}/taxon/${s.taxonID}`));
    const taxonClassificationPromises = suggestions.map(s => axios.get(`https://api.checklistbank.org/dataset/${datasetKey}/taxon/${s.taxonID}/classification`));
    Promise.all(taxonPromises).then(taxonResponses => {
      suggestions.forEach((s, i) => {
        s.taxon = taxonResponses[i].data;
      });
  ...
  return matchSorter(uniqueSuggestions, q, {keys: ['scientificName', 'vernacularName', 'acceptedNameOf']}).slice(0, limit);
}