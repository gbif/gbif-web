...
const parseNexus = (nexusString) => {
   ...
   function NexusReader() {
       ...
       this.nexusCommands = ['begin', 'dimensions', 'end', 'endblock', 'link', 'taxa', 'taxlabels', 'title', 'translate', 'tree', 'matrix', 'format'];
           ...
           while ( 
               ...
               switch (command) {
                   case 'taxlabels':
                       ...
                       nx.ParseTaxaLabels();
                       nexus.taxablock.treelabels = nx.buffer.split('\n').map(function(l) {
return l.trim();
}).filter(function(l) {
return !!l;
});
           ...
           while ( 
               ...
               switch (command) {
                   case 'matrix':
                       ...
                       nx.ParseMatrix();
                       //console.log(nx.buffer)
                       nexus.charactersblock.matrix = nx.buffer.trim().split('\n').map(function(l) {
...
return !!l;
       ...
       nexus.comments = nx.comments.filter(function(c) {
return c.charAt(1) === '!';
}).map(function(c) {
