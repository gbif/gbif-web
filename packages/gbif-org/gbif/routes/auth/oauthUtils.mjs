...
function createAccount(
  ...
  userModel
    ...
    .then(function () {
      ...
      logUserIn(res, user); // might make better sense to user the returned user from the API, but currently it only returns a fraction of the user object
      res.redirect(302, state.target || '/');
    })
    .catch(function (err) {
      next(err);
