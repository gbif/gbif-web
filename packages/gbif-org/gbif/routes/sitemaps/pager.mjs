...
async function getIntervals(url, query, limit) {
  ...
  let response = await fetchWithRetry(options.url, options);
  if (!response.ok) {
    console.log('Error fetching intervals:', response);
    throw response;
  }
  const body = await response.json();
  let ranges = _.range(0, body.count, limit);
...
async function getList(url, query) {
  ...
  let response = await fetchWithRetry(options.url, options);
  if (!response.ok) {
    console.log('Error fetching list:', response);
    throw response;
  }
