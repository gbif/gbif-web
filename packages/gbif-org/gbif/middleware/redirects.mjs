...
function createGetRedirect(env) {
  ...
  if (env.PUBLIC_STATUS_PAGE_URL) {
    newRedirects.push({
      incoming: '/system-health',
      target: env.PUBLIC_STATUS_PAGE_URL,
    });
  }
    ...
    if (queryParamRedirect) {
      ...
      if (redirectTo) {
        if (/^https?:\/\//i.test(redirectTo)) {
          // Absolute URL: forward incoming query as-is, don't prepend locale
          if (queryString) {
            redirectTo += (redirectTo.includes('?') ? '&' : '?') + queryString;
          }
        } else {
          ...
          const mergedQuery = fixParameterCasing(queryString, parameters);
          // Prepend locale prefix to redirect URL
          redirectTo = localePrefix + redirectUrl + (mergedQuery ? '?' + mergedQuery : '');
        }
      } else {
