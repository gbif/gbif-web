...
const GRAPHQL_ENDPOINT = process.env.PUBLIC_GRAPHQL_ENDPOINT || 'https://graphql.gbif.org/graphql';
...
async function main() {
  ...
  if (translations.failed.length) {
    console.log(
      `Note: ${translations.failed.length} locale(s) failed: ${translations.failed.join(', ')}`
    );
  }
}
