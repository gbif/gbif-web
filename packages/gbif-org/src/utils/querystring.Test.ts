...
test('stringify', () => {
  ...
  expect(stringify({identifier: ['https://www.gbif.org/occurrence/4535504068?test=1#anchor']})).toBe('identifier=https%3A%2F%2Fwww.gbif.org%2Foccurrence%2F4535504068%3Ftest%3D1%23anchor');
  expect(stringify({datasetKey: 'abc', limit: null, offset: undefined}), 'remove null and undefined').toBe('datasetKey=abc');
});
...
test('parse', () => {
  ...
  expect(parse('identifier=https%3A%2F%2Fwww.gbif.org%2Foccurrence%2F4535504068%3Ftest%3D1%23anchor')).toStrictEqual({identifier: 'https://www.gbif.org/occurrence/4535504068?test=1#anchor'});
