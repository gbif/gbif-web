...
export function ExploreLink({ className, projection, boundingBox }: Props) {
  const exploreAreaEnabled =
    ['EPSG_4326', 'EPSG_3857'].includes(projection) &&
    boundingBox &&
    boundingBox.right - boundingBox.left < 180;

  const searchParams = exploreAreaEnabled ? { geometry: boundingBoxToWKT(boundingBox) } : undefined;
