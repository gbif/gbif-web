...
const SearchInput = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, ...props }, ref) => {
    return (
      <div className={cn('g-inline-flex disabled:g-opacity-50 g-items-center g-justify-center g-w-full g-px-2', className)}>
        <input
        type={type}
        className={cn(
        ...
        {...props}
      />
        <button type="submit" className="g-text-slate-400 g-text-center g-flex-none g-px-1 -g-me-1 g-text-base">
          <MdSearch />
        </button>
      </div>
    );
