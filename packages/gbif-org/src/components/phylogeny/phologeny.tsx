...
export const Phylogeny = ({datasetKey, phyloTreeTipLabel, phyloTreeFileName}) => {
  const ref = useRef(null);
  const navigate = useNavigate();
  const [treeData, setTreeData] = useState({})
  const config = useConfig()
  const spacingX = 14, spacingY = 30;
  useEffect(() => {
    fetch(`${config.webApiEndpoint}/unstable-api/source-archive/${datasetKey}/phylogeny/${phyloTreeFileName}`, {
      method: 'GET',
      headers: {
    ...
    if(treeData?.nwk){
        ...
        if (tipTranslation) {
            Object.keys(tipTranslation).forEach(function(key) {
                let n = tree.getNodeByName(key);
                n.data.name = tipTranslation[key];
            });
        }

        const selectedNode = phyloTreeTipLabel ? tree.getNodeByName(phyloTreeTipLabel.replaceAll(' ', '_')) : null;
 ...
 'left-right-spacing:': 'fit-to-size',
            'node-styler': function(element: { style: (arg0: string, arg1: string, arg2: string | undefined) => void; on: (arg0: string, arg1: (e: any) => void) => void; }, data: { children: any; selected_xx: boolean; data: { name: { split: (arg0: string) => any; replaceAll: (arg0: string, arg1: string) => string; }; }; }) {
              ...
              element.on('click', function(e: any) {
               /*  if (typeof onNodeClick === 'function') {
                ...
                if (data.data.name && !data.children) {
                    ...
                    navigate(`/occurrence/search?q=${encodeURIComponent(searchTerm)}&datasetKey=${datasetKey}`)
                }
              
                });
            },
            'edge-styler': function(element: { style: (arg0: string, arg1: string, arg2: string) => void; }, data: { target: any; }) {
