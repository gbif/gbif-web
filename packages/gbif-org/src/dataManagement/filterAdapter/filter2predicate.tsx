...
export type FieldType = {
  ...
  singleValue?: boolean;
  transformValue?: (value: any) => any;
  serializer?: ({filterName, values, config}: {filterName: string, values: any[], config: FieldConfigType}) => Predicate | null;
  v1?: {
...
export function filter2predicate(filter: FilterType | undefined | null, filterConfig?: FilterConfigType): Predicate | null {
...
function getPredicates({filters, filterConfig}: { filters: Record<string, any[]> | null | undefined, filterConfig?: FilterConfigType }): Predicate[] {
  if (!filters) return [];
...
function getPredicate({ filterName, values = [], filterConfig }: {filterName: string, values: any[], filterConfig?: FilterConfigType}): Predicate | null {
