...
import { AssociateParticipantsQuery } from '@/gql/graphql';
...
const ASSOCIATE_PARTICIPANTS_QUERY = /* GraphQL */ `
  query AssociateParticipants {
    participantSearch(limit: 1000, type: OTHER, participationStatus: ASSOCIATE) {
      results {
        name
        people {
          id
...
export function AssociateParticipantsTable({ q, onPersonClick }: Props) {
  const { loading, data } = useQuery<AssociateParticipantsQuery, undefined>(
    ASSOCIATE_PARTICIPANTS_QUERY,
    {
  ...
  return (
    <Container>
      <Header
        titleId="directory.group.associateParticipants.title"
        descriptionId="directory.group.associateParticipants.description"
      />
      <Table>
        <thead>
          <tr>
...
function usePrepareData(
  data: AssociateParticipantsQuery | undefined,
  q: string,
