...
import { AssociateCountryParticipantsQuery } from '@/gql/graphql';
...
const ASSOCIATE_COUNTRY_PARTICIPANTS_QUERY = /* GraphQL */ `
  query AssociateCountryParticipants {
    participantSearch(limit: 1000, type: COUNTRY, participationStatus: ASSOCIATE) {
      results {
        name
        countryCode
        people {
...
export function AssociateCountryParticipantsTable({ q, onPersonClick }: Props) {
  const { loading, data } = useQuery<AssociateCountryParticipantsQuery, undefined>(
    ASSOCIATE_COUNTRY_PARTICIPANTS_QUERY,
    {
  ...
  return (
    <Container>
      <Header
        titleId="directory.group.associateCountries.title"
        descriptionId="directory.group.associateCountries.description"
      />
      <Table>
        <thead>
          <tr>
...
function usePrepareData(
  data: AssociateCountryParticipantsQuery | undefined,
  q: string,
