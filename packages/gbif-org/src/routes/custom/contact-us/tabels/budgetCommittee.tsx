...
import { BudgetCommitteeQuery } from '@/gql/graphql';
...
const BUDGET_COMMITTEE_QUERY = /* GraphQL */ `
  query BudgetCommittee {
    budgetCommittee {
      personId
      name
      roles
...
export function BudgetCommitteeTable({ q, onPersonClick }: Props) {
  const { loading, data } = useQuery<BudgetCommitteeQuery, undefined>(BUDGET_COMMITTEE_QUERY, {
  ...
  return (
    <Container>
      <Header
        titleId="directory.group.budgetCommittee.title"
        descriptionId="directory.group.budgetCommittee.description"
      />
      <Table>
        <thead>
          <tr>
...
function usePrepareData(
  data: BudgetCommitteeQuery | undefined,
  q: string,
  sortField: keyof Row,
  sortDirection: SortDirection
): Row[] {
  ...
  const people: Row[] = useMemo(() => {
    const members = data?.budgetCommittee ?? [];
