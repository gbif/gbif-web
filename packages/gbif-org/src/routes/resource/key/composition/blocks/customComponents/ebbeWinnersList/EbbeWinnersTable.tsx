...
import { EbbeWinnersQuery, EbbeWinnersQueryVariables } from '@/gql/graphql';
...
export function EbbeWinnersTable({
  title,
  tableStyle,
  className,
}: {
  ...
  const { data, error, loading, load } = useQuery<EbbeWinnersQuery, EbbeWinnersQueryVariables>(
    EBBE_WINNERS_QUERY,
    { lazyLoad: true }
  );
  ...
  if (error) {
    return (
      <Prose>
        <p>Error loading Ebbe Nielsen winners: {error.message}</p>
      </Prose>
    );
  }

  const winners = data?.EbbeWinners || [];
