...
export function DatasetKeyDownload() {
  let { key } = useParams();
  const { data, loading } = useParentRouteLoaderData(RouteId.Dataset) as { data: DatasetQuery };
  const { dataset } = data;
        ...
        <div className="g-flex g-flex-wrap -g-mx-4">
            ...
            <p>Be aware that an account is needed to download the content.</p>
              ...
              <CardFooter>
                <Button variant="default" className="g-text-center g-w-full" asChild>
                  <a className="g-text-white" href={`${import.meta.env.PUBLIC_GBIF_ORG}/occurrence/download/request?predicate=${encodeURIComponent(JSON.stringify(fullPredicate))}#create`}><FormattedMessage id="download.continueToGBIF" /></a>
                </Button>
              </CardFooter>
            </Card>
          </div>
          {dwcAEndpoint && <div>
            <Card className="g-flex-none md:g-w-96 g-max-w-full g-mx-4">
              ...
              <CardFooter>
                <Button variant="default" className="g-text-center g-w-full" asChild>
                  <a href={`${dwcAEndpoint.url}`} rel="noopener noreferrer" className="g-text-white">Source archive</a>
                </Button>
              </CardFooter>
            </Card>
          </div>}
