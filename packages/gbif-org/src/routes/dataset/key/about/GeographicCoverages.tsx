...
function GeographicCoverage({ coverage }) {
  ...
  if (coverage?.boundingBox?.minLatitude) {
    ...
    Bbox = <Properties useDefaultTermWidths>
      <Term><FormattedMessage id="dataset.latitude" /></Term>
      <Value>
        <FormattedMessage id="intervals.description.between" values={{ from: minLatitude, to: maxLatitude }} />
      </Value>
      <Term><FormattedMessage id="dataset.longitude" /></Term>
      <Value>
  ...
  return (
    <Properties useDefaultTermWidths>
      ...
      {!geoJSON && <>
        <Term><FormattedMessage id="dataset.boundingBox" /></Term>
        <Value>{Bbox}</Value>
      </>}
    </Properties>
  );
