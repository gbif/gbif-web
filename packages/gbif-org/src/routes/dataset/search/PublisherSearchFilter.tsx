...
import { FacetQuery } from './datasetSearch';
...
export function PublisherSearchFilter({
  ...
  facetQuery: string;
  getSuggestions: ({ q }: { q: string }) => Promise<any>;
}) {
  ...
  const facetSuggestions = facetData?.search?.facet?.field?.filter((x) => !selected.includes(x.name));
  ...
  return (
    <div className="g-flex g-flex-col g-overflow-hidden">
      <div className="g-flex g-flex-none">
        ...
        />}
        {!getSuggestions && (
          <SearchInput placeholder="Search" className="g-border-slate-100 g-border-b-2 g-py-1.5 g-px-4" onKeyDown={(e) => {
