...
export default function GeoJsonMapMaplibre({
  geojson,
  loading,
  error,
  className,
...
const Popup = ({
  ...
  const popupInstanceRef = useRef<maplibre.Popup | null>(null);
  const [anchor, setAnchor] = useState<PopupAnchor>('bottom');
  ...
  useEffect(() => {
    if (popupRef.current && lngLat && map) {
      ...
      // Detect anchor direction from MapLibre's auto-positioning.
      const popupEl = popup.getElement();
      const anchorClasses: PopupAnchor[] = [
        'top-left',
        'top-right',
        'bottom-left',
        'bottom-right',
      ...
      // Watch for class changes so the arrow updates when MapLibre
      // repositions the popup (e.g. during pan/zoom).
  ...
  return (
    <div style={{ display: 'none' }}>
      <div ref={popupRef}>
        <MapPopup anchor={anchor} onClose={() => popupInstanceRef.current?.remove()}>
          {children}
        </MapPopup>
      </div>
    </div>
