import { SearchSuggest } from '@/components/searchSelect/searchSuggest';
...
export function SingleCountryFilterSuggest({
  ...
  const { load, data } = useFetchGet<Array<CountryOption>>({
    lazyLoad: true,
  });
  ...
  const filteredData = searchTerm !== '' ? dump?.filter((x) => x.title.toLowerCase().includes(searchTerm.toLowerCase())) : dump;
