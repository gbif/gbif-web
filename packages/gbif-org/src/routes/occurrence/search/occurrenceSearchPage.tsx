  ...
  return (
    <>
      ...
      >
        ...
        */}
        <OccurrenceViewTabs setView={setView} view={view} />
      </DataHeader>
...
export function OccurrenceSearchInner(): React.ReactElement {
  const searchContext = useSearchContext();
  const { filters } = useFilters({ searchConfig });
  const [view, setView] = useStringParam({ key: 'view', defaultValue: 'media', hideDefault: true });
  ...
  return (
    <>
      ...
      <section className="g-bg-white">
        <Card>
          <OccurrenceViewTabs setView={setView} view={view} className="g-border-b" />
          <FilterBar>
            <FilterButtons filters={filters} searchContext={searchContext} />
          </FilterBar>
        </Card>
