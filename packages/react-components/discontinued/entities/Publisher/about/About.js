...
const A = Prose.A;
const { NavItem } = Toc;
export function About({ data = {}, insights, loading, error, tocState, className, ...props }) {
  ...
  const scopeSmallerThanPublisherMessage = formatMessage(
    {
      id: 'publisher.siteScopeSmallerThanPublisher',
      defaultMessage: 'Visit [GBIF.org]({publisherUrl}) to explore the full publisher.',
    },
    { publisherUrl: `https://www.gbif.org/publisher/${publisher.key}` },
  );
  ...
  const sideBarMetrics = (
    <div>
      <SideBarCard>
        <SideBarCardContentWrap>
          ...
          <div css={sharedStyles.sidebarCardContent}>
            <h5>
              <a
                href={`${env.API_V1}/occurrence/download/statistics/export?publishingOrgKey=${publisher.key}`}
              >
          ...
          {publisher.longitude && (
            ...
            >
              <img
                ...
                src={`https://api.mapbox.com/styles/v1/mapbox/streets-v9/static/pin-s-circle+285A98(${publisher.longitude},${publisher.latitude})/${publisher.longitude},${publisher.latitude},15,0/300x150@2x?access_token=${import.meta.env.PUBLIC_MAPBOX_ACCESS_TOKEN}`}
              />
            </a>
          )}
          <SideBarCardContentWrap>
      ...
      <SideBarCard>
        <SideBarCardContentWrap>
          <div css={sharedStyles.sidebarCardContent}>
            {publisher.endorsementApproved && (
              <div style={{ marginBottom: 18 }}>
                <h5>
                  Endorsed by: <A href="/country/FR">{publisher.endorsingNode.title}</A>
                </h5>
                <p>
                  Publishers need to be endorsed by a GBIF Participant Node. This endorsement
                  confirms that the publisher is a legitimate organization and that it is committed
                  to sharing biodiversity data through GBIF.
                </p>
              </div>
            )}
            ...
            {publisher.country && (
              <div style={{ marginBottom: 18 }}>
                ...
                <p>
                  The country or area where the publisher is located. For international
                  organizations, this is the country where the main office is located.
                </p>
              </div>
            )}
          </div>
    ...
    <>
      <div css={sharedStyles.withSideBar({ hasSidebar: !isBelowSidebar })}>
        <div style={{ width: '100%', overflow: 'auto' }}>
          {siteOccurrences.documents.total - total < 0 && (
            <Alert
              css={css`
                width: 100%;
                margin-top: 12px;
            ...
            >
              <HyperText
                inline
                text={scopeSmallerThanPublisherMessage}
                sanitizeOptions={{ ALLOWED_TAGS: ['a', 'strong', 'em', 'p', 'br'] }}
              />
            </Alert>
          )}

          {isBelowSidebar && <div>{sideBarMetrics}</div>}
              ...
              <CardHeader2
                ref={(node) => {
                  tocRefs['description'] = {
                    node,
                    index: 0,
                    title: <FormattedMessage id="publisher.description" />,
                  };
                }}
              >
              ...
              <CardHeader2
                ref={(node) => {
                  tocRefs['contacts'] = {
                    node,
                    index: 8,
                    title: <FormattedMessage id="publisher.contacts" />,
                  };
                }}
              >
        ...
        {!isBelowSidebar && (
          <div css={sharedStyles.sideBar}>
            ...
            <nav css={sharedStyles.sideBarNav}>
              <SideBarCard>
                {!siteContext?.publisher?.disableGbifTocLink && (
                  <ul>
                    <li>
                      <NavItem href={`${env.GBIF_ORG}/publisher/${publisher.key}`}>
                        View on GBIF.org <MdLink />
                      </NavItem>
                    </li>
                  </ul>
                )}
