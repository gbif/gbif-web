...
export function Cluster({
  ...
  const {
    ...
    load,
  } = useQuery(CLUSTER, { lazyLoad: true });
  ...
  return (
    <div style={{ padding: '12px 16px' }}>
      ...
      <div>
        <RelatedOccurrence
          onClick={(e) => setActiveKey(data.occurrence.key)}
          ...
          if (x.occurrence) {
            return (
              <RelatedOccurrence
                ...
                onClick={(e) => setActiveKey(x.occurrence.key)}
            ...
            return (
              <div style={{ padding: 30, background: 'tomato', color: 'white', borderRadius: 4 }}>
                ...
                <div>
                  <Properties style={{ marginTop: 12 }} horizontal dense>
                    ...
                    {x.stub.catalogNumber && (
                      <>
                        <T style={{ color: 'white' }}>
                          <FormattedMessage id="occurrenceFieldNames.catalogNumber" />
                        </T>
                        <V>{x.stub.catalogNumber}</V>
                      </>
                    )}
                    {x.stub.occurrenceID && (
                      <>
                        <T style={{ color: 'white' }}>
                          <FormattedMessage id="occurrenceFieldNames.occurrenceID" />
                        </T>
                        <V>{x.stub.occurrenceID}</V>
                      </>
                    )}
                  </Properties>
                </div>
...
export function RelatedOccurrence({ original, reasons, related, ...props }) {
  ...
  return (
    <article css={styles.clusterCard({ theme })} {...props}>
      <Row wrap="nowrap" halfGutter={6} style={{ padding: 12 }}>
        <Col>
          <h4
            style={{ margin: 0 }}
            dangerouslySetInnerHTML={{ __html: related.gbifClassification.usage.formattedName }}
          ></h4>
          <div css={styles.entitySummary({ theme })}>
            <IconFeatures
              css={styles.features({ theme })}
          ...
          <div style={{ fontSize: 12 }}>
            <Properties horizontal dense>
              ...
              <V>{related.publisherTitle}</V>
              <T>
                <FormattedMessage id="occurrenceDetails.dataset" />
              </T>
              <V>{related.datasetTitle}</V>
            </Properties>
            <a
              onClick={(e) => e.stopPropagation()}
              href={`${env.GBIF_ORG}/occurrence/${related.key}`}
            >
        ...
        <Col grow={false} shrink={false}>
          <div>
            ...
            {!related?.primaryImage?.identifier && related?.coordinates?.lat && (
              <img
                style={{ width: 150, height: 150, display: 'block' }}
                src={`https://api.mapbox.com/styles/v1/mapbox/light-v9/static/pin-s-circle+285A98(${related.coordinates.lon},${related.coordinates.lat})/${related.coordinates.lon},${related.coordinates.lat},8,0/180x180@2x?access_token=${import.meta.env.PUBLIC_MAPBOX_ACCESS_TOKEN}`}
              />
            )}
          </div>
        </Col>
      ...
      <div css={styles.clusterFooter({ theme })}>
        {!reasons && (
          <div>
            <FormattedMessage id="search.occurrenceClustersView.currentRecord" />
          </div>
        )}
        {reasons && (
          <Properties style={{ fontSize: 12 }} horizontal dense>
            <T>
              <FormattedMessage id="search.occurrenceClustersView.similarities" />
            </T>
            <V>
              {reasons.map((reason) => (
                <span css={styles.chip({ theme })} key={reason}>
                  <FormattedMessage
                    id={`enums.clusterReasons.${reason}`}
                    defaultMessage={prettifyEnum(reason)}
                  />
                </span>
              ))}
            </V>
