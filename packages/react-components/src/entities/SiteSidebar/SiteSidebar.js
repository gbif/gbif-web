      ...
      <Col shrink={false} grow={false} css={css.detailDrawerContent({ theme })} >
        {isLoading && (
          <Col style={{ padding: '12px', paddingBottom: 50, overflow: 'auto' }} grow>
            <h2><FormattedMessage id={`eventDetails.site`} />: {siteID} - Loading site information...</h2>
                ...
                <Row direction="column" wrap="nowrap" style={{ maxHeight: '100%', overflow: 'hidden' }}>
                  <Col style={{ paddingBottom: 50, overflow: 'auto' }} grow>
                    <Row wrap="no-wrap" css={css.header({ theme })}>
                      <Col grow>
                        <h1><FormattedMessage id={`eventDetails.site`} />: {siteID}</h1>
                        <TemporalDisplay year={year} month={month} />
                      </Col>
                    </Row>
...
export function TemporalDisplay({ year, month }) {
  if (year && month && month > 0) {
    ...
    let monthFormatted = useIntl().formatMessage({ id: `enums.month.${month}` });
    return (
      <Eyebrow 
        style={{fontSize: '80%'}}
        prefix={<><FormattedMessage id="eventDetails.activityDetails" values={{ date: monthFormatted + ', ' + year }}  /></>}
      />
    );
  } else if (year) {
    return (
      <Eyebrow 
        style={{fontSize: '80%'}}
        prefix={<FormattedMessage id="eventDetails.activityDetails" values={{ date: year }} />}
      />
    );
  }
