...
function Map() {
  ...
  const { labelMap, rootPredicate, predicateConfig, more } =
    useContext(EventContext);
  const { data, error, loading, load } = useQuery(EVENT_MAP, {
    lazyLoad: true,
    throwAllErrors: true,
    queryTag: 'map',
  });
  const {
    ...
    load: pointLoad,
  } = useQuery(EVENT_POINT, { lazyLoad: true, queryTag: 'mapPoints' });
  ...
  const loadHashAndCount = useCallback(
    ({ filter, predicateConfig, rootPredicate }) => {
      const predicate = {
        type: 'and',
        predicates: [
          rootPredicate,
  ...
  const options = {
    ...
    pointError,
    labelMap,
    defaultMapSettings: siteContext.mapSettings
