...
const fetchVocabulary = async (name, language) => {
  const vocab = (await axios.get(`http://api.gbif-uat.org/v1/vocabularies/${name}`)).data;
  const concepts = (await axios.get(`http://api.gbif-uat.org/v1/vocabularies/${name}/concepts?limit=1000`)).data;
