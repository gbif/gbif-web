              ...
              <div className="grid">
                ...
                <div className="sidebar">
                  <div className="sidebar-grid">   
                    { siteData.map( (obj, i) => <ul><li key={`s_${i}`} onClick={() => { setSiteIDCallback({locationID: obj.key}); }}> <span style={{ fontWeight:'bold'}}>{ locationIDs.get(obj.key) }</span> {obj.key?.substring(0, 18)}</li></ul>) }
                  </div>
                </div>
                <div className="main-grid">
