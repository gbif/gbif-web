...
const capabilities = testRunner({
  endpoint: `https://${config.API_V2}/map/occurrence/density/capabilities.json?taxonKey=2435099&cachebust={{NOW}}`,
...
const density = testRunner({
  endpoint: `https://${config.API_V2}/map/occurrence/density/0/0/0@1x.png?taxonKey=2435099&cachebust={{NOW}}`,
  expect: response => response
    .hasStatusCode(200)
